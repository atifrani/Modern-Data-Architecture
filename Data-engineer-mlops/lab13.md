...
def load_sample_data():
    return session.table('BEARS_DATA.STAGES.BEAR').limit(200).to_pandas()
...
if sample_data is not None:
    ...
    if st.sidebar.button("🔮 Make Prediction", type="primary"):
        ...
        with st.spinner("Making prediction..."):
            ...
            # Build inline SQL subquery to avoid temp table/view  # ✅ MODIFIÉ
            try:
                columns_sql = ", ".join(
                    [f"{v} AS {k}" for k, v in encoded_data.items()]
                )
                inline_query = f"SELECT {model_option}!PREDICT(*) AS predicted_species FROM (SELECT {columns_sql})"  # ✅ MODIFIÉ
                result = session.sql(inline_query).collect()  # ✅ MODIFIÉ
