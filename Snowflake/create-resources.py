...
def lambda_handler(event, context):
    ...
    try:
        ...
        create_storage_integration(snowflake_cursor, storage_integration_name, role_arn, s3_bucket_name)
        #create_api_integration(snowflake_cursor, api_integration_name, role_arn, api_gateway_url)
    ...
    try:
        ...
        responseData = {'Failed': 'The assume role policy could not be updated'}
        sendResponse(event, context, FAILED, responseData)
        return
...
def create_storage_integration(snowflake_cursor, storage_integration_name, role_arn, s3_bucket_name):
    ...
    storage_allowed_locations = ('s3://%s')") % (storage_integration_name, role_arn, s3_bucket_name)

    snowflake_cursor.execute(storage_integration_str)

def create_api_integration(snowflake_cursor, api_integration_name, role_arn, api_gateway_url):
    ...
    enabled = true \
    ") % (api_integration_name, role_arn, api_gateway_url)
    ...
    policy_json = {
      ...
      "Statement":[
        ...
        },
        {
          ...
          "Condition": {
            "StringEquals": {
              "sts:ExternalId": storage_integration_info["storage_external_id"]
            }
          }
        }
      ]
