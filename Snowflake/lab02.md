...
```
-- Create Databse
CREATE IF NOT EXISTS DATABASE AIRBNB;

-- Create Schema BRONZE
CREATE SCHEMA IF NOT EXISTS AIRBNB.BRONZE;

CREATE IF NOT EXISTS TABLE AIRBNB.BRONZE.HOSTS
                    (id string,
                     name string,
                     is_superhost string,
                     created_at string,
...
-- Create table
CREATE OR REPLACE TABLE AIRBNB.BRONZE.REVIEWS
                    (listing_id string,
                     date string,
...
-- Create table
CREATE OR REPLACE TABLE AIRBNB.BRONZE.LISTINGS
                    (data VARIANT
                    );
...
-- Create table
CREATE OR REPLACE TABLE AIRBNB.SILVER.FULL_MOON_DATE
(
    date_day date
);
...
```
select TRANSLATED_REVIEW_TEXT,SENTIMENT_GENERATED, SNOWFLAKE.CORTEX.EXTRACT_ANSWER(TRANSLATED_REVIEW_TEXT, 'Were the guests satisfied with their stay?'):[answer] as extract_answer from AIRBNB.GOLD.FULL_MOON_REVIEWS_AUGMENTED limit 100;
```
